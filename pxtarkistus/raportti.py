...
def poimi_taustaluvut(havainnot: pd.DataFrame, ristiin: pd.DataFrame | None) -> pd.DataFrame:
    ...
    if h.empty:
        return pd.DataFrame()
    # Yhteisraportissa sama erä, vuosi ja tunnusluku esiintyy usealla perusjoukolla;
    # taustaluvut poimitaan vain siltä joukolta, jonka havainto on kyseessä.
    joukolla = "joukko" in h.columns and "joukko" in ristiin.columns
    hj = h["joukko"] if joukolla else [""] * len(h)
    rj = ristiin["joukko"] if joukolla else [""] * len(ristiin)
    avaimet = {
        (_avain(j), _avain(e), _avain(v), _avain(t))
        for j, e, v, t in zip(hj, h["era"], h["verovuosi"], h["tunnusluku"])
    }
    r = ristiin
    maski = [
        (_avain(j), _avain(e), _avain(v), _avain(t)) in avaimet
        for j, e, v, t in zip(rj, r["era"], r["verovuosi"], r["tunnusluku"])
    ]
    poimittu = r[maski]
    sarakkeet = [
